...
http 、端口1081，点击check connection即可  

## 功能配置

profiler性能分析<https://www.jianshu.com/p/b035e2a11dd8>  
...
## 启动时选择项目
file - Settings - Appearance & Behavior - System Settings - 取消勾选Reopen last project on startup选项    
...
点这个就能看到详细的错误了  
一般都是因为引入的三方包存在相同的label icon 等名字或者是sdk版本不一致  
...
### AS导入NDK工程提示错误 No such property: sdkHandler for class: com.android.build.gradle.LibraryPlugin
```
...
得到的答案是所有的路径都要用英文。就是包括SDK JDK ADT Eclipse 还是工程存储位置，所有的路径都要改成英文。   
...
### 安装andriod studio时出现Internal error. Please report to https://code.google.com/
```
有两种方法的哈
...
替换gradle-wrapper.properties 文件里的gradle版本
